...
async function embed_procedure() {
	...
	for(let i = 0; i < pixel_length; i++, !(i % 512) && await new Promise(setTimeout)) {
		if(unsolve_gv[cover_image.gv[i]] == 0) {
			let pixel = embed_can(cover_image.r[i], cover_image.g[i], cover_image.b[i], i);
	...
	for(let i = 0; i < pixel_length; i++, !(i % 512) && await new Promise(setTimeout)) {
		...
		switch(unsolve_gv[cover_image.gv[i]]) {
			case 0:
				pixel = embed_can(pixel.r, pixel.g, pixel.b, i);
				break;
			case 1:
				pixel = embed_not(pixel.r, pixel.g, pixel.b, i);
				break;
		}
		...
		marked_image.data[i*4 + 3] = image_data[i*4 + 3];
	...
	for(let i = 0; i < pixel_length; i++, !(i % 512) && await new Promise(setTimeout)) {
		...
		if(i < 128) {
			...
			if(result.r == 0) {
				...
				detect_image.data[i*4 + 3] = Math.min(60, image_data[i*4 + 3]);
			}
		}
...
function embed_can(r, g, b, index) {
